-- ============================================================================
-- COMMUNICATION FEATURES MIGRATION (FIXED)
-- Adds SMS, voice call capabilities with rate limiting and monitoring
-- ============================================================================

-- Add new event types to contact timeline (skip if already exists)
DO $$ 
BEGIN
  BEGIN
    ALTER TYPE contact_event_type ADD VALUE 'sms_sent';
  EXCEPTION
    WHEN duplicate_object THEN null;
  END;
  
  BEGIN
    ALTER TYPE contact_event_type ADD VALUE 'sms_received';
  EXCEPTION
    WHEN duplicate_object THEN null;
  END;
  
  BEGIN
    ALTER TYPE contact_event_type ADD VALUE 'voice_call_made';
  EXCEPTION
    WHEN duplicate_object THEN null;
  END;
END $$;
