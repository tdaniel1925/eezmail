...
interface UseSentEmailsOptions {
  limit?: number;
  enabled?: boolean;
}
...
/**
 * Custom hook for fetching and caching sent emails with SWR
 ...
 */
export function useSentEmails(options: UseSentEmailsOptions = {}) {
  const { limit = 25, enabled = true } = options;

  const { data, error, isLoading, isValidating, mutate } =
    useSWR<SentEmailsResponse>(
      enabled ? `/api/email/sent?limit=${limit}` : null,
      async (url: string) => {
