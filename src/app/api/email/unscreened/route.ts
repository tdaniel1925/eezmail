...
// Validation schema for unscreened emails request
const unscreenedEmailSchema = z.object({
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Validate parameters
    const validatedData = unscreenedEmailSchema.parse({ limit, offset });

    console.log(`📧 Fetching unscreened emails for user:`, user.id);
    ...
    console.log(`📧 Found unscreened emails:`, unscreenedEmails.length);

    return NextResponse.json({
      success: true,
      emails: unscreenedEmails,
      total,
      hasMore,
      limit: validatedData.limit,
      offset: validatedData.offset,
    ...
    console.error('❌ Error fetching unscreened emails:', error);
    ...
    return NextResponse.json(
      { error: 'Failed to fetch unscreened emails' },
      { status: 500 }
    );
  }
}
