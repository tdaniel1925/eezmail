...
import { getSpamEmails } from '@/lib/email/get-emails';

interface AutoSyncSpamProps {
  accounts: any[];
  title?: string;
}

export function AutoSyncSpam({
  accounts,
  title = 'Spam',
}: AutoSyncSpamProps) {
  ...
  const { isSyncing, lastSyncAt, syncCount, triggerSync } = useAutoSync({
    ...
    intervalMs: 300000, // 5 minutes (less frequent for spam)
  ...
  const fetchEmails = async () => {
    try {
      setIsLoading(true);
      const result = await getSpamEmails(100); // Get more spam emails
      ...
      if (result.success) {
        ...
        setError(result.error || 'Failed to fetch spam emails');
      }
    } catch (err) {
