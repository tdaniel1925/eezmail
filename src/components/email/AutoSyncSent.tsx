...
interface AutoSyncSentProps {
  accountId: string;
}

export function AutoSyncSent({ accountId }: AutoSyncSentProps) {
  ...
  const fetchSentEmails = async () => {
    try {
      setIsLoading(true);
      const result = await getEmailsByFolder('sent');
      ...
      <div className="flex items-center justify-between p-4 border-b bg-gradient-to-r from-green-50 to-teal-50">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-lg font-semibold text-green-800">Sent</h2>
              <p className="text-sm text-green-600">Your sent emails</p>
            </div>
          </div>
          ...
          {newEmailsCount > 0 && (
            <div className="flex items-center gap-2 bg-green-100 text-green-800 px-3 py-1 rounded-full text-sm font-medium animate-pulse">
              <span>📤</span>
              <span>
                {newEmailsCount} new sent email{newEmailsCount > 1 ? 's' : ''}
              </span>
            </div>
          )}
          ...
          <div className="flex items-center gap-2">
            {isSyncing ? (
              <div className="flex items-center gap-2 text-green-600">
                ...
                <span className="text-sm">Syncing sent...</span>
              </div>
            ) : (
              <div className="flex items-center gap-2 text-green-600">
        ...
        >
          {isSyncing ? 'Syncing...' : 'Refresh Sent'}
        </button>
      </div>
